...
wishlistRouter.post("/items", async (req, res, next) => {
    try {
        ...
        const hasItem = wishlist.items.some((item) => item.productId.toString() === input.productId);
        if (!hasItem) {
...
wishlistRouter.delete("/items/:productId", async (req, res, next) => {
    try {
        ...
        const updateResult = await WishlistModel.updateOne({ userId: principal.userId }, {
            $pull: {
