...
adminRouter.get("/dashboard/summary", async (_req, res, next) => {
    try {
        const [totalUsers, totalCustomers, totalAdmins, blockedUsers, totalOrders, recentOrders, totalRevenue, recentRevenue, orderStatusRows,] = await Promise.all([
    ...
    try {
        ...
        if (isNewlyCancelled) {
            await Promise.all(order.items.map((item) => {
                const update = item.size && item.color
                    ? {
                        $inc: {
                ...
                const options = item.size && item.color
                    ? {
                        arrayFilters: [
                    ...
                    : {};
                return ProductModel.updateOne({ _id: item.productId }, update, options);
            }));
        }
...
function requireAdminPrincipal(res) {
