...
const homepageCacheKeys = [HOMEPAGE_CACHE_KEY];
export async function getHomepageCache() {
...
export async function setHomepageCache(payload) {
    const redis = getRedisClient();
    if (!redis) {
        return;
    }
...
export async function invalidateHomepageCache(reason) {
    const redis = getRedisClient();
    if (!redis) {
        return;
    }
...
//# sourceMappingURL=homepage-cache.service.js.map