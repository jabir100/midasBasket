...
export function adminNewOrderTemplate(order) {
    return {
        ...
        html: layout("New order received", `${text(`${order.customerName} (${order.customerEmail}) just placed a new order.`)}
...
export function orderConfirmationTemplate(order) {
    return {
        ...
        html: layout("Thank you for your order!", `${text(`Hi ${order.customerName}, we've received your order and it's being prepared.`)}
       ${strongLine(`Order ${order.orderNumber}`)}
       ${itemsTable(order.items, order.currency)}
       ${strongLine(`Total: ${formatMoney(order.total, order.currency)}`)}
...
export function orderStatusUpdateTemplate(order) {
    ...
    return {
        ...
        html: layout(copy.subject, `${text(`Hi ${order.customerName}, ${copy.message}`)}
       ${strongLine(`Order ${order.orderNumber}`)}
       ${itemsTable(order.items, order.currency)}
       ${button("Track your order", `${CLIENT_BASE_URL}/track`)}`),
    };
...
export function welcomeTemplate(input) {
    return {
        ...
        html: layout(`Welcome, ${input.name}!`, `${text(`Thanks for creating an account with ${BRAND_NAME}. You're all set to start shopping.`)}
...
export function passwordResetRequestedTemplate(input) {
    ...
    return {
        ...
        html: layout("Reset your password", `${text(`Hi ${input.name}, we received a request to reset your password. This link expires in 1 hour.`)}
...
export function passwordChangedTemplate(input) {
    return {
        ...
        html: layout("Password changed", `${text(`Hi ${input.name}, your password was just changed and all active sessions were signed out.`)}
...
export function accountStatusChangedTemplate(input) {
    ...
    return {
        ...
        html: layout(isBlocked ? "Account blocked" : "Account reactivated", `${text(`Hi ${input.name}, your account has been ${isBlocked ? "blocked" : "reactivated"} by an administrator.`)}
...
export function roleChangedTemplate(input) {
    ...
    return {
        ...
        html: layout(isAdmin ? "Admin access granted" : "Admin access removed", `${text(`Hi ${input.name}, your account role was changed to <strong>${input.role}</strong> by an administrator.`)}
