...
function formatValidationMessage(details, fallbackMessage) {
    const messages = [];
    if (details.formErrors) {
        messages.push(...details.formErrors.filter((message) => message.trim().length > 0));
    }
    if (details.fieldErrors) {
        for (const [field, fieldMessages] of Object.entries(details.fieldErrors)) {
            ...
            const firstMessage = fieldMessages.find((message) => message.trim().length > 0);
            if (firstMessage) {
...
export const errorHandler = (error, _req, res, _next) => {
    ...
    if (error instanceof Error && error.name === "ValidationError") {
        ...
        const details = {
            fieldErrors: Object.fromEntries(Object.entries(mongooseError.errors ?? {}).map(([field, validationError]) => [
                field,
    ...
    if (error instanceof Error && error.code === 11000) {
        ...
        sendError(res, {
            ...
            details: {
                fieldErrors: Object.fromEntries(getDuplicateKeyFields(duplicateKeyError).map((field) => [
                    field,
                    ["Already exists"],
                ])),
            },
