...
import {
  ...
  setHomepageCache,
} from "./homepage-cache.service.js";
import {
  BlogModel,
...
export const homepageRouter: ExpressRouter = Router();
          ...
          settings?.whyChooseUs && settings.whyChooseUs.length > 0
            ...
            : [
                ...
                },
                {
                  ...
                  description:
                    "SSR-friendly sections, cache-backed homepage, lean interactions, and semantic HTML.",
                },
                {
                  id: "curated",
...
adminRouter.patch("/carousel/:id/toggle", async (req, res, next) => {
  try {
    ...
    const slide = await CarouselSlideModel.findByIdAndUpdate(
      req.params.id,
      { $set: { isActive } },
      { new: true }
    );
